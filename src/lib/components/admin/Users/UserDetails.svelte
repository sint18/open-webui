<script lang="ts">
    ...
    import { getTransactions, getPaymentOrders, getUserCreditsByUserId } from '$lib/apis/billing';
    ...
    onMount(async () => {
        ...
        try {
            const [creditsData, transactionsData, ordersData, userInfo] = await Promise.all([
                ...
                getPaymentOrders(localStorage.token),
            ...
            {#if credits}
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <Badge
                            type={credits.status === 'active' ? 'success' : credits.status === 'grace' ? 'warning' : 'error'}
                    ...
                    <table class="min-w-full">
                        ...
                        <tbody>
                            {#each orders as order}
                                <tr class="border-t dark:border-gray-800">
                                    <td class="py-2">
                                        {dayjs(order.created_at * 1000).format('MMM D, YYYY')}
                                    </td>
