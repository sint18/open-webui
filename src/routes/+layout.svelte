	...
	const channelEventHandler = async (event) => {
		...
		if ((!channel || isFocused) && event?.user?.id !== $user?.id) {
			...
			if (type === 'message') {
				if ($isLastActiveTab) {
					if ($settings?.notificationEnabled ?? false) {
						new Notification(`${data?.user?.name} (#${event?.channel?.name}) • Laby AI`, {
							body: data?.content,
...
<!--	Umami Analytics -->
	<script defer src="https://umami-zc84g0wow04wsgsks04cwsoc.innovaxweb.com/script.js" data-website-id="15dde480-a21f-4645-a00c-f3467d798d8a"></script>
