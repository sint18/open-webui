<script lang="ts">
	...
	let links: Link[] = [];
	let coupons: Coupon[] = [];
	let commissions: {items: Commission[], footer: string} = { items: [], footer: ''};
	let payouts: Payout[] = [];
...
<div class="space-y-6 text-gray-800 dark:text-gray-200">
	{#if partner}
		<div class="relative z-10 space-y-2">
			...
			<div>
				...
				<pre
					class="bg-gray-100 dark:bg-gray-900 p-2 rounded overflow-x-auto">{JSON.stringify(partner.payout_details, null, 2)}</pre>
			</div>
			<div>
				<p class="font-semibold">{$i18n.t('Rates')}</p>
				<pre
					class="bg-gray-100 dark:bg-gray-900 p-2 rounded overflow-x-auto">{JSON.stringify(partner.rates, null, 2)}</pre>
			</div>
		</div>
	{/if}
	...
	<section>
		...
		{#if loading}
			<DataGridSkeleton />
		{:else if links.length === 0 || links.length <= 0}
	...
	<section>
		...
		{#if loading}
			<DataGridSkeleton />
		{:else if coupons.length === 0 || coupons.length <= 0}
	...
	<section>
		...
		{#if loading}
			<DataGridSkeleton />
		{:else if !commissions || commissions.items.length <= 0}
			<EmptyState title={$i18n.t('No commissions')} />
		{:else}
			<DataGrid columnDefs={commissionCols} rowData={commissions.items} {gridOptions} />
		{/if}
	</section>
	...
	<section>
		...
		{#if loading}
			<DataGridSkeleton />
		{:else if payouts.length === 0 || payouts.length <= 0}
	...
	<section>
		...
		{#if loading}
			<DataGridSkeleton />
		{:else if payoutItems.length === 0 || payoutItems.length <= 0}
...
<Modal bind:show={showEdit}>
	...
	<div class="p-4 space-y-4 text-gray-800 dark:text-gray-200">
		...
		<div class="space-y-2">
			...
			<input
				class="w-full p-2 border rounded bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 border-gray-300 dark:border-gray-700"
		...
		<div class="space-y-2">
			...
			<input
				class="w-full p-2 border rounded bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 border-gray-300 dark:border-gray-700"
				bind:value={editForm.payout_method} />
		</div>
		<div class="space-y-2">
			<label class="block text-sm">{$i18n.t('Terms Version')}</label>
			<input
				class="w-full p-2 border rounded bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 border-gray-300 dark:border-gray-700"
				bind:value={editForm.terms_version} />
		</div>
		<div class="space-y-2">
			<label class="block text-sm">{$i18n.t('Payout Details')}</label>
			<textarea
				class="w-full p-2 border rounded font-mono text-sm h-24 bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 border-gray-300 dark:border-gray-700"
				bind:value={payoutDetailsText}></textarea>
		</div>
		<div class="space-y-2">
			<label class="block text-sm">{$i18n.t('Rates')}</label>
			<textarea
				class="w-full p-2 border rounded font-mono text-sm h-24 bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 border-gray-300 dark:border-gray-700"
				bind:value={ratesText}></textarea>
		</div>
		<div class="flex justify-end gap-2">
			<button class="px-3 py-1 rounded bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200"
							on:click={() => (showEdit = false)}>{$i18n.t('Cancel')}</button>
